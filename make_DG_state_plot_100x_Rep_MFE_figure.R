...
MFE.dumpfile.dir <- args[3]
EQ.MFE.dumpfile.dir <- args[4]
CO.MFE.dumpfile.dir <- args[5]
title <- args[6]
...
CO.MFE.dumpfiles <- list.files(CO.MFE.dumpfile.dir, pattern="\\.dump$", full.names=TRUE)
for(CO.MFE.dumpfile in CO.MFE.dumpfiles){
 CO.MFE.data <- read.table(CO.MFE.dumpfile, header=TRUE, sep="\t")
 lines(CO.MFE.data$nt, CO.MFE.data$DG, col="deeppink", lwd=0.6)
}
...
for(MFE.dumpfile in MFE.dumpfiles){
 ...
 lines(MFE.data$nt, MFE.data$DG, col="black", lwd=0.6)
}
...
for(EQ.MFE.dumpfile in EQ.MFE.dumpfiles){
 ...
 lines(EQ.MFE.data$nt, EQ.MFE.data$DG, col="blue", lwd=0.6)
}
